  ...
  saveForm(): void {
    if (this.personalDetailsFormGroup.valid) {
      const formDetails: PatientPersonalDetailsModel = {
        ...
        mobileNumber: this.personalDetailsFormGroup.controls.mobileNumber.value.replace(/\s+/g,' ').trim(),
        alternateMobileNumber: this.personalDetailsFormGroup.controls.alternateMobileNumber.value.replace(/\s+/g,' ').trim(),
